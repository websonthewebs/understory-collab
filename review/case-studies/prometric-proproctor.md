# Scaling Remote Proctored Exams for Prometric with Twilio Flex and Programmable Video

_When the pandemic shut down testing centers worldwide, several Understory team members — then at Greyshore Associates — helped Prometric rebuild its remote proctoring platform in under four months._

---

### At a Glance

| Field | Value |
|---|---|
| **Client** | Prometric |
| **Industry** | Testing & Assessment |
| **Engagement** | Greyshore Associates (Twilio partner) |
| **Understory Connection** | Several Understory team members contributed to this work during their time at Greyshore |
| **Areas of Contribution** | Delivery Ecosystem (Infrastructure), Front-End Application, User Experience Design, Accessibility, Security Consulting |
| **Timeline** | June – October 2020 |
| **Key Technologies** | Twilio Flex, Twilio Programmable Video |
| **Outcome** | 80% increase in concurrent test session capacity |

---

### The Challenge

Prometric is one of the world's largest testing and assessment providers, administering high-stakes professional and academic exams at centers around the globe. When in-person testing halted in early 2020, demand for remote proctoring surged overnight.

Prometric's existing remote assessment platform, ProProctor, wasn't built to handle the volume. Students facing final exams and professionals pursuing certifications needed to test from home — and the platform had to deliver the same security, reliability, and human support they'd expect at a physical testing center.

The stakes were high in every sense: exam integrity, candidate experience, and Prometric's reputation as an industry leader all depended on getting this right, fast.

---

### The Work

In June 2020, Prometric engaged Greyshore Associates — a Twilio partner — alongside Twilio's Professional Services team to rapidly re-launch ProProctor on a modern, scalable foundation. The work spanned multiple disciplines, with Understory team members contributing across four key areas.

**Delivery Ecosystem — Infrastructure**

The delivery ecosystem team built and maintained the infrastructure underpinning ProProctor — the foundation that everything else ran on. [Placeholder — what did this include specifically? CI/CD, cloud infrastructure, environments, deployment pipelines? Flag for team review.]

**Front-End Application**

The Greyshore team built a custom UI on Twilio Flex that gave proctors a unified view of everything they needed: candidate video feeds, chat, exam status, and support tools — all in a single interface designed to Prometric's exact specifications. Rather than toggling between systems, proctors could monitor and support candidates from one screen.

**User Experience Design**

[Placeholder — what did UX design look like on this project? Was the team designing the proctor experience, the candidate experience, or both? How did the compressed timeline shape design decisions? Flag for team review.]

**Accessibility**

[Placeholder — what accessibility work was involved? WCAG compliance for the candidate-facing experience? Proctor tooling? Were there specific challenges around making a video-first proctoring platform accessible? Flag for team review.]

**Security Consulting**

For a platform handling high-stakes professional and academic exams, security was non-negotiable. [Placeholder — what did security consulting involve? Threat modeling, penetration testing, secure architecture review, compliance requirements? How did security considerations shape the platform design? Flag for team review.]

**Programmable Video — Exam Integrity at Scale**

Without in-person supervision, video became the backbone of exam integrity. Twilio Programmable Video provided the live connection between candidates and proctors — enabling face-to-face interaction for questions or issue resolution while maintaining a reliable, clear feed throughout the exam session.

**Speed as a Constraint**

The entire platform was re-launched in under four months. [Placeholder — what made this possible? Architecture decisions, team structure, delivery approach? Flag for team review.]

---

### The Results

| Outcome | Detail |
|---|---|
| **Concurrent Capacity** | 80% increase in concurrent test sessions |
| **Delivery Timeline** | Full platform re-launch in under four months |
| **Proctor Experience** | Unified "single pane of glass" UI replacing multi-system workflow |
| **Candidate Experience** | Reliable video and real-time support for high-stakes remote exams |
| **Analytics** | Flex Insights enabled rich analytics on testing experience and performance |
| **Industry Recognition** | Featured as a [Twilio customer story](https://customers.twilio.com/en-us/prometric) |

---

### Client Perspective

> [Placeholder — action item: check whether Prometric has provided any public quotes beyond the Twilio case study]

---

### Reflections

[Placeholder — what did the team learn from this engagement? Possible angles: delivering under extreme time pressure during a global crisis, the dynamics of working as a Twilio partner alongside Twilio's own PS team, scaling video infrastructure for high-stakes use cases. Flag for team members who worked on this to contribute.]

---

### Key Takeaways

- **Crisis creates urgency — and opportunity.** The pandemic forced a complete rethink of remote proctoring, and the tight timeline demanded focused execution with no room for scope creep.
- **Platform choices matter.** Building on Twilio Flex and Programmable Video gave Prometric a customizable, scalable foundation rather than a rigid off-the-shelf solution.
- **Video is infrastructure, not a feature.** In high-stakes remote testing, video reliability directly affects exam integrity and candidate trust — it's not a nice-to-have.
- [Placeholder — additional takeaway from the team's perspective? Flag for review.]

---

### Related Services

- [Implementation — Full-Stack Development](/implementation)
- [Advisory — Strategic Consulting](/advisory)
- [Contact — Start a Conversation](/contact)

---

### Review Notes

_Items for the author to revisit before publishing:_

- [ ] Fill in Delivery Ecosystem (Infrastructure) details — what specifically was built?
- [ ] Fill in User Experience Design details — proctor UX, candidate UX, or both?
- [ ] Fill in Accessibility details — WCAG compliance, specific challenges?
- [ ] Fill in Security Consulting details — threat modeling, pen testing, compliance?
- [ ] Fill in what made the fast delivery timeline possible
- [ ] Fill in the Reflections section with input from team members
- [ ] Check whether Prometric has any public quotes available
- [ ] Confirm the June–October 2020 timeline
- [ ] Review framing of Greyshore/Understory connection
